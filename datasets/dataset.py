...
class WeatherDataset(Dataset):
    def __init__(self, file_path, input_seq_len=8, target_seq_len=1, var_name='data'):
        self.ds = xr.open_dataset(file_path)
        self.val_name = var_name
        self.input_seq_len = input_seq_len
        self.target_seq_len = target_seq_len
        self.total_seq_len = input_seq_len + target_seq_len
        self.max_idx = self.ds.dims['day'] - self.total_seq_len + 1 # 假设时间维度叫day
    
    def __len__(self):
        return self.max_idx
    
    def __getitem__(self, idx):
        seq = self.ds[self.val_name][idx:idx+self.total_seq_len].values
