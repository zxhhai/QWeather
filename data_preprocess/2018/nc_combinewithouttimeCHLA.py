...
# 配置设置
SOURCE_DIR = Path(r"C:\RIP_D\Codes\Python\QWeather\DataProcess\datas\data_m\par_m")
YEAR = 2018
OUTPUT_FILE = f"combined_par_{YEAR}_full_year.nc"
CHUNK_SIZE = 50  # 每批处理的文件数

logger.info(f"开始合并 CHLA 数据集...")
...
with xr.open_dataset(sample_path) as sample_ds:
    ...
    # 创建CHLA变量
    chla = ds_out.createVariable('CHLA', 'f4', ('time', 'lat', 'lon'), 
                               zlib=True, complevel=4, fill_value=-9999.0)
    chla.units = sample_ds.CHLA.units if 'units' in sample_ds.CHLA.attrs else 'mg m-3'
    chla.long_name = sample_ds.CHLA.long_name if 'long_name' in sample_ds.CHLA.attrs else 'Chlorophyll-a concentration'
    
    # 设置全局属性
    ds_out.title = f"Combined CHLA Data for {YEAR}"
    ...
    for i, date in enumerate(chunk_dates):
        ...
        if date in file_info:
            ...
            if pos == 0:
                ...
                nearest_date = available_dates[pos - 1]
        ...
        try:
            ...
            with xr.open_dataset(source_path, chunks={'Latitude': 1000}) as ds:
                # 确保数据维度匹配
                if ds.CHLA.shape != (lat_dim, lon_dim):
                    logger.warning(f"文件 {source_path.name} 维度不匹配: {ds.CHLA.shape} vs ({lat_dim}, {lon_dim})")
                
                # 加载数据到内存
                chla_data = ds.CHLA.values
                
                # 写入输出文件
                ds_out['CHLA'][idx, :, :] = chla_data
            ...
            # 使用填充值
            ds_out['CHLA'][idx, :, :] = np.full((lat_dim, lon_dim), -9999.0)
            processed_count += 1
...
try:
    ...
    with xr.open_dataset(OUTPUT_FILE) as final_ds:
        # 输出摘要信息
        logger.info(final_ds)
        logger.info(f"时间维度大小: {len(final_ds.time)}")
        
        # 检查数据质量
        valid_count = np.sum(final_ds.CHLA.values != -9999.0)
        ...
        for date in all_dates:
            ...
            chla_data = final_ds.CHLA.isel(time=idx)
            
            # 计算缺失值比例
            missing_percent = np.mean(chla_data == -9999.0) * 100
