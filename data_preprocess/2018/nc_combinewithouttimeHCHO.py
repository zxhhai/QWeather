...
# 重新获取变量引用
hcho_var = ds_out['HCHO']
aux_vars_refs = {}
for var_name in aux_vars:
    aux_vars_refs[var_name] = ds_out[var_name]
...
for chunk_start in range(0, total_days, CHUNK_SIZE):
    ...
    for i, date in enumerate(chunk_dates):
        ...
        try:
            # 使用分块读取避免内存问题 - 使用更合适的块大小
            with xr.open_dataset(source_path, chunks={'latitude': 900, 'longitude': 1800}) as ds:
                # 处理主变量 (HCHO)
                ...
                for var_name in aux_vars_refs:
                    if var_name in ds.data_vars:
                        ...
                        # 写入辅助变量
                        aux_vars_refs[var_name][idx, :, :] = aux_data
                    else:
            ...
            # 使用填充值
            hcho_var[idx, :, :] = np.full((lat_dim, lon_dim), -9999.0)
            # 填充辅助变量
            for var_name in aux_vars_refs:
                aux_vars_refs[var_name][idx, :, :] = np.full((lat_dim, lon_dim), -9999.0)
            processed_count += 1
